/*
  ...
  Update 21 augustus 2020
   - Typefout in de functiedefinitie "forecast_shortterm()" opgelost. Dit was eerst met een enkele 't' in plaats van dubbel 't'.

  Update 11 februari 2021
   - Als er nog geen huidige locatie was, werd de functie showWeatherInfo() soms niet correct aangeroepen. Dit is verholpen.
*/
...
//extra added
function currentLocation_temperature_in_CDegree(id){
    var nearestWeatherStationData = getNearestWeatherStation("temperature");
    if(nearestWeatherStationData.temperature === undefined) {
        document.getElementById(id).innerHTML = "temperatuur onbekend";
        return false;
    } else {
        document.getElementById(id).innerHTML = nearestWeatherStationData.temperature + " &#8451";
        return true;
    }
}
...
function get_date_showDate(datum = new Date(), seperator = "-") {
...
// Voorspellingen
function forecast_shortterm(id) {
...
function get_forecast_minTemperature( dagen = 1) {
    dagen = validateDayNumber(dagen);
    if(buienradarData.forecast.fivedayforecast.length === 0) {
        return msgNoData;
    }
    return buienradarData.forecast.fivedayforecast[dagen - 1].mintemperature;
}


function get_forecast_maxTemperature( dagen = 1) {
    dagen = validateDayNumber(dagen);
    if(buienradarData.forecast.fivedayforecast.length === 0) {
        return msgNoData;
    }
    return buienradarData.forecast.fivedayforecast[dagen - 1].maxtemperature;
}


function get_avg_temp(dagen = 1){
    return ( parseInt(get_forecast_maxTemperature(dagen)) + parseInt(get_forecast_minTemperature(dagen)) ) / 2;
}
...
function get_forecast_rainChance(dagen = 1) {
    dagen = validateDayNumber(dagen);
    if(buienradarData.forecast.fivedayforecast.length === 0) {
        return msgNoData;     
    }
...
function get_forecast_sunChance( dagen = 1) {
    dagen = validateDayNumber(dagen);
    if(buienradarData.forecast.fivedayforecast.length === 0) {
        return msgNoData;
    }
...
function get_forecast_windDirection(dagen = 1) {
    dagen = validateDayNumber(dagen);
    if(buienradarData.forecast.fivedayforecast.length === 0) {
       return msgNoData;
    }
   return buienradarData.forecast.fivedayforecast[dagen - 1].windDirection.toUpperCase();
} 
...
function get_forecast_wind(dagen = 1) {
    dagen = validateDayNumber(dagen);
    if(buienradarData.forecast.fivedayforecast.length === 0) {
        return msgNoData;
    }
    return buienradarData.forecast.fivedayforecast[dagen - 1].wind;
}
...
function get_forecast_iconurl(dagen = 1) {
    dagen = validateDayNumber(dagen);
    if(buienradarData.forecast.fivedayforecast.length === 0) {
    return msgNoData;
    }
